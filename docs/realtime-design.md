...
A hardware test run of 104 events measured min=12us, p95=17us, p99=18us, but
max=5051us, and that outlier is a known, accepted, event-0-only artifact
...
is only reachable via a transition that already released all keys), but
would conflict with the blanket, transition-independent safety invariant
above, which deliberately does not depend on that cross-transition reasoning
continuing to hold under future changes -- not worth trading a documented
safety guarantee for shaving off an already-negligible artifact. Lowering
the HID descriptor's `bInterval` below 10ms would shrink the worst case, but
would change USB bus characteristics for every report this device ever
sends just to fix one bounded, first-event-only artifact; 10ms is a
standard, conservative interval for a boot-protocol keyboard. A third,
non-blocking option -- deferring `playback_start_us` sampling until after
the release clears the endpoint -- was also considered and not pursued: it
would not reduce any real delay, only relabel which reported number absorbs
it.
