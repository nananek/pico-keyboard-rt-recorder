...
| 4 | Zero recorder CLI and persistence | Capture sessions persist valid recordings with list/dump operations. |
| 5 | UART v1 parser, Pico playback queue, and flow control | Queue commands/status and queue tests operate correctly on the version-2 transport from Phase 3. |
...
| 7 | UART-based mode gate and safe Pico state machine | UART0 RX ring, main-loop version/CRC parser, `MODE_SET`, `MODE_CHANGED`, and all-release transitions are verified. |
...
| 9 | Reliability and RT test suite | Underrun, disconnect, persistent-underrun watchdog, load/stall, and jitter tests pass. |
